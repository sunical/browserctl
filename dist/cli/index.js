...
function parseDuration(value) {
    ...
    if (!match)
        throw new Error(`Invalid timeout: "${value}". Use e.g. 30m, 1h, 5000ms`);
...
program
    ...
    .action(async (opts) => {
    const sessionId = await resolveSession(opts);
    const result = await api(`/sessions/${sessionId}/screenshot`, { fullPage: opts.fullPage !== false });
    if (!result.success) {
        console.error(result.error);
        process.exit(1);
    }
...
program
    ...
    .action(async (opts) => {
    const sessionId = await resolveSession(opts);
    const result = await api(`/sessions/${sessionId}/a11y`, {});
    if (!result.success) {
        console.error(result.error);
        process.exit(1);
    }
