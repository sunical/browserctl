 ...
 * satisfied.
 */
export async function waitFor(page, condition, timeout = DEFAULT_TIMEOUT_MS) {
...
export function parseWaitCondition(args) {
    const chosen = [];
    if (args.forSelector)
        chosen.push({ kind: 'selector', selector: args.forSelector });
    if (args.forGone)
        chosen.push({ kind: 'gone', selector: args.forGone });
    if (args.forText)
        chosen.push({ kind: 'text', text: args.forText });
    if (args.forNetworkIdle)
        chosen.push({ kind: 'networkIdle' });
    if (args.forNavigation)
        chosen.push({ kind: 'navigation' });
    if (chosen.length > 1) {
    ...
    if (args.ms === undefined || Number.isNaN(args.ms)) {
        throw new Error('wait needs a duration or a condition, e.g. "wait 500" or "wait --for-selector .results"');
    }
