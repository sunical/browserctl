...
const str = (v: Args[string]): string => (v === undefined ? '' : String(v))
const num = (v: Args[string]): number => Number(v)
...
export const commands: Record<string, CommandSpec> = {
  ...
  keys: {
    ...
    numeric: ['repeat'],
    run: (page, a) => keys(page, str(a.method) as 'press' | 'type', str(a.value), a.repeat ? num(a.repeat) : 1),
  },
  scroll: {
    mutating: true,
    positional: ['direction'],
    numeric: ['percent'],
    run: (page, a) => scroll(page, str(a.direction) as 'up' | 'down', a.percent ? num(a.percent) : 80),
  },
  drag: {
    mutating: true,
  ...
  fillform: {
    ...
    positional: ['fields'],
    run: (page, a) =>
      fillform(page, typeof a.fields === 'string' ? parseFields(a.fields) : (a.fields as never)),
  },
  wait: {
    ...
    run: (page, a) =>
      waitFor(
        ...
        parseWaitCondition({
          ...
          forSelector: a['for-selector'] as string | undefined,
          forText: a['for-text'] as string | undefined,
          forGone: a['for-gone'] as string | undefined,
...
export async function executeCommand(page: Page, name: string, args: Args): Promise<unknown> {
...
function splitSteps(script: string): string[] {
  ...
  if (quote) throw new Error(`Unterminated ${quote} quote in script`)
  if (current.trim()) steps.push(current.trim())
  return steps
}
 ...
 */
export function parseScript(script: string): ParsedStep[] {
  return splitSteps(script).map(source => {
    const tokens = tokenize(source)
    const name = tokens.shift()
    if (!name) throw new Error(`Empty step in script: "${source}"`)
